...
class Ma3Cross(CtaTemplate):
    ...
    normalMa = []             # 与上面相同
    normalMa0 = EMPTY_FLOAT
    normalMa1 = EMPTY_FLOAT
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(Ma3Cross, self).__init__(ctaEngine, setting)
        ...
        fast_list = [initial_value]*fastK
        normal_list = [initial_value]*normalK
        slow_list = [initial_value]*slowK
        
    #----------------------------------------------------------------------
    def onInit(self):
        """初始化策略（必须由用户继承实现）"""
        self.writeCtaLog(u'3MACross演示策略初始化')
    ...
    def onStart(self):
        """启动策略（必须由用户继承实现）"""
        self.writeCtaLog(u'3MACross演示策略启动')
        self.putEvent()
    
    #----------------------------------------------------------------------
    def onStop(self):
        """停止策略（必须由用户继承实现）"""
        self.writeCtaLog(u'3MACross演示策略停止')
        self.putEvent()
    ...
    def onBar(self, bar):
        ...
        fastArrayFlag = countNum % fastK
        normalArrayFlag = countNum % normalK
        slowArrayFlag = countNum % slowK
        countNum += 1;

        fast_list[fastArrayFlag] = bar.close
        normal_list[normalArrayFlag] = bar.close
        slow_list[slowArrayFlag] = bar.close

        fast_sum = 0
        elementIsFull = 1
        for element in fast_list:
            fast_sum += element 
            if element == 0:
                elementIsFull = 0
        if elementIsFull == 1:
            fast_num = fast_sum/fastK
            if not self.fastMa0:        
                self.fastMa0 = fast_num
        ...
        for element in normal_list:
            ...
            if element == 0:
                elementIsFull = 0
        if elementIsFull == 1:
            normal_num = normal_sum/normalK
            if not self.normalMa0:
                self.normalMa0 = normal_num
                self.normalMa.append(self.normalMa0)
            else:
                self.normalMa1 = self.normalMa0
                self.normalMa0 = normal_num
                self.normalMa.append(self.normalMa0)

        slow_sum = 0
        elementIsFull = 1
        for element in slow_list:
            slow_sum += element
            if element == 0:
                elementIsFull = 0
        if elementIsFull == 1:
            slow_num = slow_sum/slowK   
            if not self.slowMa0:
                self.slowMa0 = slow_num
        ...
        # 判断买卖
        crossOver = self.fastMa0>self.normalMa0 and self.normalMa0>self.slowMa0 and (self.fastMa1<normalMa1 or self.normalMa1<self.slowMa1 or self.fastMa1<self.slowMa1)     # 金叉上穿
        crossBelow = self.fastMa0<self.normalMa0 and self.normalMa0<self.slowMa0 and (self.fastMa1>self.normalMa1 or self.normalMa1>self.slowMa1 or self.fastMa1>self.slowMa1)    # 死叉下穿
